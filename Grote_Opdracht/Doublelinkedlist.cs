...
namespace Grote_Opdracht
{
    ...
    class DoubleLinkedList
    {
        ...
        public static DoubleLinkedList KiesRandomOphaalPatroon(List<DoubleLinkedList> huidigeOphaalpatronen, out int index)
        {
            Random random = new Random();
            if (huidigeOphaalpatronen == null || huidigeOphaalpatronen.Count == 0)
            {
                throw new ArgumentException("De lijst met ophaalpatronen mag niet leeg zijn.");
            }

            index = random.Next(0, huidigeOphaalpatronen.Count);
            return huidigeOphaalpatronen[index];
        }
        ...
        public int IndexOf(Bedrijf bedrijf)
        {
            Node current = head;
            int index = 0;
            while (current != null)
            {
                if (current.data.Equals(bedrijf))
                {
                    return index;
                }
                current = current.next;
